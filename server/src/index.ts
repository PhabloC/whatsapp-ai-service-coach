...
// Permitir CORS apenas de origens específicas (mais seguro)
...
whatsappManager.on(
  "message",
  (instanceId: string, message: WhatsAppMessage) => {
    console.log(
      `📤 Enviando mensagem via WebSocket para instância ${instanceId}:`,
...
app.post("/api/instances/:id/profile-pictures", async (req, res) => {
  try {
    ...
    if (!jids || !Array.isArray(jids)) {
      return res
        .status(400)
        .json({ error: 'Campo "jids" deve ser um array de JIDs' });
    }

    const results = await whatsappManager.getProfilePictures(
      id,
      jids,
      highRes === true,
    );
...
app.get("/api/instances/:id/contacts/:jid", async (req, res) => {
  try {
    ...
    if (!contactInfo) {
      return res
        .status(404)
        .json({ error: "Não foi possível obter informações do contato" });
    }
...
app.post("/api/sessions/:id/restore", async (req, res) => {
  try {
    ...
    if (!instance) {
      return res
        .status(404)
        .json({ error: "Sessão não encontrada ou não autenticada" });
    }
