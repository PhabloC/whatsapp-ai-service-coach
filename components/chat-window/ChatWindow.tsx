...
import {
  analyzeConversation,
  analyzeConversationWithHeatmap,
  generateSalesScript,
} from "@/geminiService";
...
import { showErrorToUser } from "@/src/utils/error-handler";
...
export const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  const [selectedHeatmapId, setSelectedHeatmapId] = useState<string | null>(
    null,
  );
  const [showEvolutionHistory, setShowEvolutionHistory] = useState(false);
  const [analysisMode, setAnalysisMode] = useState<"classic" | "heatmap">(
    "heatmap",
  );
  const [showSalesScriptModal, setShowSalesScriptModal] = useState(false);
  const [isGeneratingScript, setIsGeneratingScript] = useState(false);
  const [currentSalesScript, setCurrentSalesScript] =
    ...
    if (session) {
      setCurrentPrompt(
        session.customPrompt ||
          "Analise o tom de voz e a eficiência em resolver o problema do cliente.",
      );
      setSelectedHistoryId(
        session.analysisHistory.length > 0
          ? session.analysisHistory[0].id
          : null,
      );
      setSelectedHeatmapId(
        session.heatmapHistory && session.heatmapHistory.length > 0
          ? session.heatmapHistory[0].id
          : null,
      );
    }
  }, [session]);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      showErrorToUser(error, "Erro ao analisar conversa");
    } finally {
  ...
  const handleAnalyzeHeatmap = async () => {
    ...
    if (
      !criteriaToUse ||
      !(Object.values(criteriaToUse) as string[]).some((v) => v.trim() !== "")
    ) {
      alert(
        "Por favor, configure os critérios de avaliação primeiro na aba AJUSTES > Critérios de Avaliação.",
      );
      return;
    }
    ...
    try {
      const result = await analyzeConversationWithHeatmap(
        session.messages,
        criteriaToUse,
      );
      ...
      if (onSaveAnalysis) {
        const analysisEntry: AnalysisEntry = {
          ...
          result: {
            ...
            strengths: Object.values(result.analise).map(
              (p) => p.justificativa,
            ),
            improvements: [],
      ...
      showErrorToUser(error, "Erro ao gerar Heatmap Score");
    } finally {
  ...
  const handleMarkAsSale = async () => {
    ...
    try {
      ...
      showErrorToUser(error, "Erro ao gerar script de vendas");
  ...
  const activeAnalysis = session?.analysisHistory.find(
    (h) => h.id === selectedHistoryId,
  )?.result;
  const activeHeatmap = session?.heatmapHistory?.find(
    (h) => h.id === selectedHeatmapId,
  )?.analysis;
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-slate-100 text-slate-400">
        ...
        <p className="text-lg font-medium">
          Selecione uma conversa para começar o coaching
        </p>
      </div>
    );
  }
      ...
      <div className="flex-1 flex flex-col bg-[#e5ddd5] dark:bg-slate-900 overflow-hidden relative">
        <div className="p-4 bg-white border-b flex items-center justify-between z-10 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="relative w-10 h-10 rounded-full bg-emerald-100 overflow-hidden">
              ...
              <span
                className={`avatar-fallback absolute inset-0 flex items-center justify-center text-emerald-600 font-bold ${session.profilePicture ? "hidden" : ""}`}
              >
            ...
            <div>
              ...
              <p className="text-xs text-emerald-500 font-medium">
                Conexão Ativa • {session.messages.length} mensagens
              </p>
            </div>
          </div>
        ...
        {isEditingConfig && (
          <div className="absolute top-20 right-4 left-4 bg-white shadow-2xl rounded-2xl p-6 z-20 border border-emerald-100 animate-fade-in">
            <h4 className="font-bold text-slate-800 mb-2">
              Configurar Prompt desta Conexão
            </h4>
            <textarea
              value={currentPrompt}
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={() => {
                  onUpdateSessionPrompt(session.id, currentPrompt);
                  setIsEditingConfig(false);
                }}
                className="px-6 py-2 bg-emerald-500 text-white rounded-lg font-bold"
              >
                Salvar Configuração
              </button>
            </div>
        ...
        >
          {session.messages.map((m) => (
            <div
              key={m.id}
              className={`flex ${m.sender === "agent" ? "justify-end" : "justify-start"} animate-fade-in`}
            >
              <div
                className={`max-w-[70%] p-3 rounded-xl shadow-sm ${m.sender === "agent" ? "bg-[#dcf8c6]" : "bg-white"}`}
              >
      ...
      <div className="w-96 border-l bg-white flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {analysisMode === "heatmap" && activeHeatmap ? (
            ...
            session.heatmapHistory &&
            session.heatmapHistory.length > 0 ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  Histórico de Heatmaps
                </label>
              </div>
              <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                {session.heatmapHistory.map((h) => (
                  ...
                  >
                    ...
                    {h.analysis.nota_final.toFixed(1)}
                  </button>
                ))}
              </div>
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center p-8 opacity-40">
              ...
              {!session.criteriaConfig && (
                <p className="text-xs text-amber-600 mt-2">
                  Configure os critérios primeiro!
                </p>
              )}
              <button
                onClick={handleAnalyzeHeatmap}
                className="mt-4 text-xs font-bold text-emerald-500 uppercase"
              >
                Gerar Heatmap
              </button>
            </div>
          ...
          {analysisMode === "classic" && activeAnalysis ? (
            <div className="space-y-6 animate-fade-in">
              <div className="text-center p-6 bg-slate-50 rounded-2xl border border-dashed border-slate-300">
                <div
                  className={`text-5xl font-black mb-2 ${activeAnalysis.score >= 70 ? "text-emerald-500" : activeAnalysis.score >= 50 ? "text-amber-500" : "text-red-500"}`}
                >
                ...
                {session.analysisHistory.length > 1 &&
                  selectedHistoryId === session.analysisHistory[0].id && (
                    <div className="mt-3 pt-3 border-t border-slate-200">
                      {(() => {
                        const prevScore =
                          session.analysisHistory[1]?.result.score || 0;
              ...
              <div className="space-y-3">
                ...
                <ul className="space-y-2">
                  {activeAnalysis.strengths.map((s, i) => (
                    <li
                      key={i}
                      className="text-sm text-slate-600 bg-emerald-50 p-3 rounded-xl border border-emerald-100 flex gap-2"
                    >
              ...
              <div className="space-y-3">
                ...
                <ul className="space-y-2">
                  {activeAnalysis.improvements.map((s, i) => (
                    <li
                      key={i}
                      className="text-sm text-slate-600 bg-amber-50 p-3 rounded-xl border border-amber-100 flex gap-2"
                    >
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center p-8 opacity-40">
              ...
              <p className="text-sm font-medium">
                Nenhuma análise realizada para esta sessão.
              </p>
              <button
