...
const STAGE_INFO: Record<
  "idle" | "scanning" | "authenticating" | "syncing" | "connected",
  StageInfo
> = {
  idle: {
    title: "Aguardando",
  ...
  scanning: {
    title: "QR Detectado",
    description: "Lendo informações do dispositivo...",
    progress: 25,
  },
  authenticating: {
    title: "Autenticando",
    description: "Verificando credenciais de segurança...",
    progress: 50,
  },
  syncing: {
    title: "Sincronizando",
    description: "Carregando conversas e contatos...",
    progress: 75,
  },
  connected: {
    title: "Conectado!",
    description: "Instância pronta para monitoramento",
    progress: 100,
  },
};
...
export const QRCodeScanner: React.FC<QRCodeScannerProps> = ({
  ...
  useEffect(() => {
    ...
    const handleQR = (data: { instanceId: string; qrCode: string }) => {
      ...
      // SEMPRE aceitar novos QRs do backend - o Baileys gera novos QRs automaticamente quando o anterior expira
      // Não bloquear novos QRs mesmo se já existe um válido, pois o QR anterior pode ter expirado no servidor
      setCountdown((prevCountdown) => {
        setQrCode(data.qrCode);
        return 120; // Sempre resetar countdown quando receber novo QR
      });
    ...
    const createInstance = async () => {
      ...
      try {
        ...
        const instance = await whatsappAPI.createInstance(
          `Instância ${new Date().toLocaleTimeString()}`,
        );
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-900 p-6">
      <div className="w-full max-w-lg bg-white p-8 rounded-3xl shadow-2xl text-center space-y-6 border border-slate-100 animate-fade-in">
        ...
        <div className="flex justify-center gap-2">
          {(
            ["scanning", "authenticating", "syncing", "connected"] as (
              | "idle"
              | "scanning"
              | "authenticating"
              | "syncing"
          ...
          <div className="bg-slate-50 rounded-2xl p-4 space-y-3 text-left">
            ...
            <ol className="text-xs text-slate-500 space-y-2">
              <li className="flex items-start gap-2">
                <span className="w-5 h-5 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0">
                  1
                </span>
              ...
              <li className="flex items-start gap-2">
                ...
                <span>
                  Toque em Menu (⋮) ou Configurações e selecione "Dispositivos
                  conectados"
                </span>
              </li>
              <li className="flex items-start gap-2">
